...
/// Messages sent from [`CallbackHandler`] to the resolver [`Actor`](super::Actor).
///
...
pub(super) enum EngineMessage {
    ...
    /// A fetch failed (timeout, peer disconnect, etc.).
    /// The actor removes all pending subscribers for the key.
    Failed { key: Request },
...
impl resolver::Consumer for CallbackHandler {
    type Key = Request;
    type Value = Bytes;
    type Failure = ();
    ...
    async fn failed(&mut self, key: Self::Key, _: Self::Failure) {
        self.sender.send_lossy(EngineMessage::Failed { key }).await;
    }
}

impl Producer for CallbackHandler {
    type Key = Request;
...
mod tests {
    ...
    fn response_decode_rejects_invalid_pinned_flag() {
        ...
        *encoded
        ...
        let err = match Response::<u64, sha256::Digest>::decode(Bytes::from(encoded)) {
