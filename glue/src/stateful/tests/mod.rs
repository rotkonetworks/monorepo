...
fn sync_then_participate() {
    ...
    // After 5 finalizations, the harness starts validator 0. Its
    // simplex engine receives finalization certificates from peers.
    // Each finalization triggers lazy recovery: the actor walks back
    // through the block DAG (via marshal, which backfills from
    // peers) and replays forward to rebuild pending state. After
    // catching up, validator 0 joins consensus. All validators must
    // agree on committed state.
    let engine = ConsensusEngine::new(NUM_VALIDATORS);
    let databases = engine.databases.clone();
    ...
    assert_all_committed_states_agree(&databases, "after delayed join");
}
...
fn sync_then_participate_after_large_gap() {
    ...
    assert_all_committed_states_agree(&databases, "after large-gap delayed join");
}
